...
def main():
    ...
    cement = st.number_input('cement:', min_value=50.0,
                             max_value=1000.0, value=50.0)

    blast_furnace_slag = st.number_input(
        'blast_furnace_slag:', min_value=0.1, max_value=400.0, value=1.0)

    fly_ash = st.number_input(
        'fly_ash:', min_value=0.1, max_value=250.0, value=1.0)

    water = st.number_input('water:', min_value=100.0,
                            max_value=250.0, value=100.0)

    superplasticizer = st.number_input(
        'superplasticizer:', min_value=0.1, max_value=35.0, value=1.0)

    coarse_aggregate = st.number_input(
        'Coarse_aggregates:', min_value=700.0, max_value=1200.0, value=700.0)

    fine_aggregate = st.number_input(
        'Fine_aggregates:', min_value=500.0, max_value=1000.0, value=500.0)
    ...
    if st.button("Predict"):
        result = predict(cement, blast_furnace_slag, fly_ash, water,
